...
pre code {
  ...
  overflow: auto;
		...
		function addNote({ color }) {
			// 真正执行 添加标注 的代码, 先添加标注框，框中内容此时不赋值，只在最后保存时赋值
			...
			// 一旦触发 加入 note函数 之后，需要在document上加入标签
		...
		function getContent(compressHTML, updatedResources) {
			...
			doc.querySelectorAll(`template[${SHADOWROOT_ATTRIBUTE_NAME}]`).forEach(templateElement => {
				...
				// 保存摘要
				const abstract = mainElement.value;
				const parentElement = templateElement.parentElement;
				const parentNodeName = parentElement.localName;
				const id = parentElement.attributes[0].nodeValue;
				// 摘要对应的标签 name  <single-file-note data-note-id="2"></single-file-note>
